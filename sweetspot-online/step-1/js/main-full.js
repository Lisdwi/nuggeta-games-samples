...
function sendSelectedColumn (row, column) {
    ++shotCount;
    selectedColumn = new SelectedColumn().init();
    selectedColumn.setColumn(column);
    log.info("sending position to oponent");
...
function sendThinking (column) {
    if(game != null){
       thinking  = new Thinking().init();
       thinking.setColumn(column);
...
$("#facebook_log").click(function() {
    nuggetaPlug.thirdPartyLogin(LoginSource.FACEBOOK);
});
...
$("#games").click(function() {
  showScreen("game_list_page");

  var nuggetaQuery = new NuggetaQuery();
  nuggetaQuery.setLimit(100);
  nuggetaQuery.setQuery("$WHERE DisconnectedContext.Participants $ELEM_MATCH {ID = '" + loggedPlayer.getID() + "'}");
...
$("#win_submit_score").click(function() {
    ...
    if (loggedPlayer != null) {
        nuggetaPlug.submitScoreRequest(scoreValue, "ld1", function(response) {
            nuggetaPlug.publishThirdPartyMessage(loggedPlayer.getThirdPartyId(), "SweetSpot online", "I made a score of " + scoreValue + " points", null);
        });
    } else {
