...
function _startMove(column){
    ...
    if(row >= 6)
		...
		log.info("#active player = " + SweetSpot.activeplayer) ;
		if(SweetSpot.activeplayer ==0){
		    ignore_user_input();
		
		    sendSelectedColumn(row, column);
		}
...
function game_over(player) {
  ...
  var playerPage = function () {
      ...
      $('#choose_color_page').delegate('.pick', 'mousedown', function (e) {
		...
		if (color == 3 && !greenColorUnlocked) {
		    return;
		}
		
		if (color == 0 && !orangeColorUnlocked) {
		    return;
		}
...
var gameTypePage = function () {
	...
	if (immediateJoin) {
	    nuggetaPlug.gameApi().matchAndJoinGame(null, null, 60000);
...
function sendSelectedColumn (row, column) {
    ++shotCount;
    selectedColumn = new NRawGameMessage().init();
    selectedColumn.setContent("s:" + column);
    log.info("sending position to opponent");

    nuggetaPlug.sendGameMessage(selectedColumn, game.getId());
    nuggetaPlug.gameApi().nextPlayerTurn(game.getId());
}

function sendThinking (column) {
    if(game != null){
       thinking  = new NRawGameMessage().init();
       thinking.setContent("t:"+ column);
       nuggetaPlug.sendGameMessage(thinking, game.getId());
   }
}
...
function sendAchieve () {
    /*if (!greenColorUnlocked) {
        nuggetaPlug.achieveRequest("ACH_SWSP_1", function (response) {
            if (response.getAchieveStatus() == AchieveStatus.SUCCESS) {
                $("#win_page").hide();
                showScreen("achievement_page");
                $("#achievement_title").html("Congratulations!<br/>You win a new color!");
                $("#achievement_img").html('<img src="images/3Dcandy-green.png" />');

                greenColorUnlocked = true;
                $("#player_player1green").removeClass('locked');
            }
        });
    }*/
}
...
function onGetThirdPartyFriendResponse(message) {
    ...
    if (message.getGetThirdPartyFriendsStatus() == GetThirdPartyFriendsStatus.SUCCESS) {
        ...
        $('.challenge_link').click(function () {
            nuggetaPlug.challengeThirdPartyFriend($(this).attr('name'), "I want play with you", "Come playing SweetSpot 4 in a row game online with me. ", "http://games.nuggeta.com/sweetspot/connected/images/3Dcandy-blue128.png","http://games.nuggeta.com/sweetspot/connected/index.html",loggedPlayer.getLoginSource());
        });
    }
}
...
function endGame() {
    ...
    nuggetaPlug.gameApi().unjoinGame(game.getId());
}

function playAgain() {
    //nuggetaPlug.gameApi().unjoinGameRequest(game.getId(), function (response) {
        nuggetaPlug.gameApi().matchAndJoinGame(null, null, 60000);
    //});
}
...
function onMatchAndJoinGameResponse(message) {
    game = message.getGame();

    if(message.getMatchAndJoinGameStatus() == MatchAndJoinGameStatus.ACCEPTED){
        initFirstGame();
    } else {
        invalidJoinGame();
    }
...
function onSelectedColumn(col) {
    log.info("opponent played " + col);
    SweetSpot.activeplayer = 1;
    _startMove(col);
}

function onThinking(col) {
    _updateSelector(col);
}



function onThirdPartyLoginResponse(message) {
    if (message.getThirdPartyLoginStatus() == ThirdPartyLoginStatus.CONNECTED) {
        showScreen("logged_menu");

        loggedPlayer = message.getPlayer();

        $('#player1name').val(loggedPlayer.getName());
...
function onChallengeAccepted(message) {
    gameIdToJoin = message.getGameId(); 
    immediateJoin = false;
...
function onGetChallengesResponse(message) {
    ...
    if (message.getGetChallengesStatus() == GetChallengesStatus.SUCCESS) {
        var challenges = message.getChallenges();
        if (challenges.length > 0) {
            for (var i = 0; i < challenges.length; i++) {
                var avatarUrl = nuggetaPlug.getAvatar(challenges[i].getRequester());
                $('#challenges_list').append('<div class="challenge_item"><table style="width:100%;"><tr><td><img src="' + avatarUrl + '"/> ' + challenges[i].getRequester().getName() + '</td><td style="text-align:right;"> <div class="accept_link" name="' + challenges[i].getId() + '">Accept</div></td></tr></table></div>');
            }

            $('.accept_link').click(function () {
                nuggetaPlug.acceptFriendChallenge($(this).attr('name'));
            }); 
        } else {
            $('#challenges_list').append('<div id="no_challenge_found">No challenges found</div>');
        }

    }
}

function onAcceptFriendChallengeResponse(message) {
    if (message.getAcceptFriendChallengeStatus() == AcceptFriendChallengeStatus.SUCCESS) {
        gameIdToJoin = message.getGameId(); 
        immediateJoin = false;
...
function onGetLeaderboardResponse(message) {
    $('#scores_list').html("");
    if (message.getGetLeaderboardStatus() == GetLeaderboardStatus.SUCCESS) {
        $("#leaderboard_name").html(message.getLeaderboard().getName());

        var scores = message.getLeaderboard().getScores();
        for (var i = 0; i < scores.length; i++) {
            var avatarUrl = '';
            if (scores[i].getPlayer().getThirdPartyId() != null) {
                avatarUrl = nuggetaPlug.getAvatar(scores[i].getPlayer());
            }
            $('#scores_list').append('<div class="score_item"><table style="width:100%;"><tr><td><img src="' + avatarUrl + '" /><span class="score_name">' + scores[i].getPlayer().getName() + '</span></td><td style="text-align:right;">' +  scores[i].getValue() + ' points</td></tr></table></div>');
        }
    }
}
...
function onGetAchievementBoardResponse(message) {
    if (message.getGetAchievementBoardStatus() == GetAchievementBoardStatus.SUCCESS) {
        var achievements = message.getAchievementBoard().getAchievements();
        for (var i = 0; i < achievements.length; ++i) {
            if (achievements[i].getId() == "ACH_SWSP_1" && achievements[i].isAchieved()) {
                greenColorUnlocked = true;
                $("#player_player1green").removeClass('locked');
            }
        }
...
$("#facebook_log").click(function() {
    nuggetaPlug.thirdPartyLogin(ThirdPartySource.FACEBOOK);
});

$("#logged_menu_back").click(function() {
    nuggetaPlug.logout();
});

$("#quick_match").click(function() {
    immediateJoin = true;
...
$("#challenge_friend").click(function() {
  ...
  nuggetaPlug.getThirdPartyFriends(ThirdPartySource.FACEBOOK);
});
...
$("#challenges").click(function() {
  ...
  var nuggetaQuery = new NuggetaQuery();
  nuggetaQuery.setLimit(100);
  nuggetaQuery.setQuery("$WHERE Target.ID = '" + loggedPlayer.getID() + "'");
  nuggetaPlug.getChallenges(nuggetaQuery);
});
...
$("#scores").click(function() {
  showScreen("scores_page");

  nuggetaPlug.getLeaderboard("ld1", LeaderboardPeriod.ALL, 0, 10);
});

$("#fb_scores").click(function() {
  showScreen("scores_page");

  nuggetaPlug.getLeaderboard("ld1", LeaderboardPeriod.ALL, 0, 10);
});

$("#score_page_back").click(function() {
    backToPreviousScreen();
});  

$("#win_submit_score").click(function() {
    var scoreValue = ((42 - shotCount) * 1234);
    if (loggedPlayer != null) {
        nuggetaPlug.submitScoreRequest(scoreValue, "ld1", function(response) {
        
        nuggetaPlug.publishThirdPartyMessage("SweetSpot online", "I made a score of " + scoreValue + " points",  "http://games.nuggeta.com/sweetspot/connected/images/3Dcandy-blue128.png","http://games.nuggeta.com/sweetspot/connected/index.html",loggedPlayer.getLoginSource());
        });
    } else {
        var nick = prompt("Please enter your name:", "");
        nuggetaPlug.submitScoreWithPlayerNameRequest(nick, scoreValue, "ld1", function(response) {
...
$("#achievement_back_button").click(function() {
    backToPreviousScreen();
});
...
var greenColorUnlocked = false;
var orangeColorUnlocked = false;
...
var pump = function pump(){
    messages = nuggetaPlug.pumpAndGet();
    if(messages.length >0){
        for(var i = 0; i < messages.length; i++) {
            message = messages[i];
            log.info("received message "  + message.toString());

		      if(message instanceof MatchAndJoinGameResponse){
                  onMatchAndJoinGameResponse(message);
			...
			else if(message instanceof SelectedColumn){
			    onSelectedColumn(message);
			}else if(message instanceof Thinking){
			    onThinking(message);
			}

			else if(message instanceof ThirdPartyLoginResponse){
			    onThirdPartyLoginResponse(message);
			}else if(message instanceof GetThirdPartyFriendsResponse) {
			    onGetThirdPartyFriendResponse(message);
			} else if (message instanceof LogoutResponse) {
			    onLogoutResponse(message);
			}

			else if(message instanceof ChallengeAccepted) {
			    onChallengeAccepted(message);
			}else if(message instanceof ChallengeThirdPartyFriendResponse) {
			    onChallengeThirdPartyFriendResponse(message);
			}else if(message instanceof GetChallengesResponse) {
			    onGetChallengesResponse(message);
			}else if(message instanceof AcceptFriendChallengeResponse) {
			    onAcceptFriendChallengeResponse(message);
			}

			else if(message instanceof GetLeaderboardResponse) {
			    onGetLeaderboardResponse(message);
			}


			else if (message instanceof GetAchievementBoardResponse) {
			    onGetAchievementBoardResponse(message);
			}*/
...
$("#intro_playbutton").html("Connecting...");
    var nuggetaPlug = new NuggetaGamePlug().init("127.0.0.1:5010");

     // Register API

    nuggetaPlug.setStartResponseHandler(function(startresponse){
     ...
     nuggetaPlug.gameApi().addMatchAndJoinGameResponseHandler(onMatchAndJoinGameResponse);
     nuggetaPlug.gameApi().addGameRunningStateChangeHandler(onGameStateChange);
     nuggetaPlug.gameApi().addMyTurnNotificationHandler(onMyTurnNotification);
     nuggetaPlug.gameApi().addPlayerUnjoinGameHandler(onPlayerUnjoinGame);

     nuggetaPlug.gameApi().addNRawGameMessageHandler(function (message) {

         var props = message.getContent().split(':');
         if (props[0] == 't') {
             onThinking(parseInt(props[1]));
         } else if (props[0] == 's') {
             onSelectedColumn(parseInt(props[1]));
         }
     });
